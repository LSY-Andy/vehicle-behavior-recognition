...
def load_label_map(file_path):
    ...
    for x in lines:
        tempd[int(x[0])] = x[1]
#     tempd={1:"stand",2:"sit",3:"walk",4:"ride_safe",5:"ride_danger"}
    return tempd
#     return{stand,sit,walk,ride_safe,ride_danger}
#     return {int(x[0]): x[1] for x in lines}
#     return {int(round(float(x[0]))): int(float(x[1])) for x in lines}
